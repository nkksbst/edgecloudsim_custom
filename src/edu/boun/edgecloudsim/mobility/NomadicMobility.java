...
public class NomadicMobility extends MobilityModel {
	...
	public void initialize() {
		...
		Document doc = SimSettings.getInstance().getEdgeDevicesDocument(); // parse from edge_devices.xml
		NodeList datacenterList = doc.getElementsByTagName("datacenter"); // get list of data centers
		for (int i = 0; i < datacenterList.getLength(); i++) {
			...
			// this creates an array of exp distribution per datacenter centered at mean waiting time
			// each datacenter will have different waiting time
			expRngList[i] = new ExponentialDistribution(SimSettings.getInstance().getMobilityLookUpTable()[placeTypeIndex]);
		}

		//initialize tree maps and position of mobile devices
		// treeMapArray contains treemap for each device
		// each tree map is a series of location and time
