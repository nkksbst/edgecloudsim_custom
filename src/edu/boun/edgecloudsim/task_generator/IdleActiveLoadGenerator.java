...
public class IdleActiveLoadGenerator extends LoadGeneratorModel{
	...
	public void initializeModel() {
		...
		// array of exp distrubition [number_of_apps][3] why 3? because each app's task has three characteristics, see below
		...
		for(int i=0; i<SimSettings.getInstance().getTaskLookUpTable().length; i++) {
			if(SimSettings.getInstance().getTaskLookUpTable()[i][0] ==0)
				continue;
			// for each app i, get upload, download, length
		...
		for(int i=0; i<numberOfMobileDevices; i++) {
			...
			// this determines what type of app will run on the mobile device i
			...
			taskTypeOfDevices[i] = randomTaskType; // each device will get a random app task
			...
			while(virtualTime < simulationTime) {
				...
				// as long as virtual time is still within the active period region, keep creating tasks
