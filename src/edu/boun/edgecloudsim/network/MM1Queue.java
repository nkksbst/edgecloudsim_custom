...
public class MM1Queue extends NetworkModel {
	...
	public void initialize() {
		...
		for (int i=0; i<SimSettings.getInstance().getTaskLookUpTable().length; i++) {
			double weight = SS.getTaskLookUpTable()[i][0]/(double)100; //usage_percentage
			if(weight != 0) {
				WlanPoissonMean += (SS.getTaskLookUpTable()[i][2])*weight; // usage_percentage * poisson_mean

				double percentageOfCloudCommunication = SS.getTaskLookUpTable()[i][1];　// how likely the app will offload to cloud
	...
	public double getUploadDelay(int sourceDeviceId, int destDeviceId, Task task) {
		double delay = 0;
		// find the access point/ datacenter associated with the device
	...
	public double getDownloadDelay(int sourceDeviceId, int destDeviceId, Task task) {
		...
		//edge device (wifi access point) to mobile device
			...
			EdgeHost host = (EdgeHost)(SimManager.
					...
					getHostList().get(0)); // there is only one host in each edge device
	...
	private double calculateMM1(double propagationDelay, int bandwidth /*Kbps*/, double PoissonMean, double avgTaskSize /*KB*/, int deviceCount){
		...
		mu = Bps / avgTaskSize ; //task per seconds // service rate
