...
def plot_generic_result(row_offset, col_offset, y_label, app_type, calculate_percentage):
    ...
    for folder_path in list_output:
        for s in range(num_simulations):
            for i in range(len(scenario_type)):
                for j in range(num_mobile_devices):
                    ...
                    file_path = folder_path + '/default_config' + '/ite' +  str(s + 1) + '/SIMRESULT_TWO_TIER_WITH_EO_' + scenario_type[i] +  '_' + str(mobile_device_number) + 'DEVICES_' + app_type + '_GENERIC.log'
        ...
        #plt.figure().clear()
        plt.plot(x_index, results[0,:], label = os.path.basename(folder_path))
