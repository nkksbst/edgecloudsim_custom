...
def plot_generic_result(row_offset, col_offset, y_label, app_type, calculate_percentage):
    ...
    num_simulations = get_configuration(2)
    mobile_device_loop_start = get_configuration(3)
    mobile_device_loop_step = get_configuration(4)
    mobile_device_loop_end = get_configuration(5)
    x_tick_label_coefficient = get_configuration(6)

    scenario_type = get_configuration(7)
    legends = get_configuration(8)
    ...
    for s in range(num_simulations):
        for i in range(len(scenario_type)):
            for j in range(num_mobile_devices):
                ...
                file_path = folder_path + '/ite' +  str(s + 1) + '/SIMRESULT_' + scenario_type[i] +  '_NEXT_FIT_' + str(mobile_device_number) + 'DEVICES_' + app_type + '_GENERIC.log'
